...
#### f152001 Local Human Lights Product
...
# open prepared data
avg <- raster(paste0("data/downloaded/nightlight/avg/F152001.v4b_web.avg_vis.tif"))
pct <- raster(paste0("data/downloaded/nightlight/pct/F152001.v4b.pct_lights.tif"))
lma <- raster(paste0("data/prepared/nightlight/local_variables/lm_freq_5_f152001.tif"))
lmb <- raster(paste0("data/prepared/nightlight/local_variables/lm_avg_25_f152001.tif"))
lmc <- raster(paste0("data/prepared/nightlight/local_variables/lm_freq_99_f152001.tif"))
lmd <- raster(paste0("data/prepared/nightlight/local_variables/lm_avg_199_f152001.tif"))
lme <- raster(paste0("data/prepared/nightlight/local_variables/local_noise_f152001.tif"))
lmf <- raster(paste0("data/prepared/nightlight/local_variables/local_detections_f152001.tif"))
...
raster.list$fun <- mean
mos <- do.call(mosaic, raster.list)
writeRaster(mos, paste0("data/final_products/LHLmask_average_f152001.tif"), format="GTiff", overwrite=TRUE)
...
writeRaster(LHL_mask, "data/final_products/LHL_mask_f152001.tif", format="GTiff", overwrite=TRUE)

LHL_mask = raster(paste0("data/final_products/LHL_mask_f152001.tif"))
avg_light <- raster(paste0("data/downloaded/nightlight/avg/F152001.v4b_web.avg_vis.tif"))
avg_light = crop(avg_light,extent(LHL_mask))
Local_Human_Lights_version_1 <- overlay(LHL_mask, avg_light, fun=function(x,y){(x*y)} )
writeRaster(Local_Human_Lights_version_1, "data/final_products/Local_Human_Lights_f152001.tif", format="GTiff", overwrite=TRUE)
